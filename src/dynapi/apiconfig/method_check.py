...
class ParsedSection:
    def __init__(self, section: str):
        ...
        self.schema_re = re.compile(translate_globby(self.schema)) if self.schema else None
        self.table = parts[2] if len(parts) > 2 else None
        self.table_re = re.compile(translate_globby(self.table)) if self.table else None
    ...
    def match_table(self, table) -> bool:
        if not self.table:
            return True
...
def method_check(*, method: str, schema: str, table: str) -> bool:
    for section in ordered_sections():
        if section.match_schema(schema) and section.match_table(table):
